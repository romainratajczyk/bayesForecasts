...
parameters {
  ...
  vector[D_v] rho_raw; // passer en K_clusters, 72 000 paramètres mal identifiés c'est pas super. Sinon, laisser D_v
...
transformed parameters {
  ...
  // 1ere raison: stricte positivité, 2: hiérarchie additionnelle en echelle log.
  // ZTNB_log prend eta=exp(lambda) en paramètre, lambda=exp(ar_pred) est la moyenne de la ZTNB sous-jacente
  // neg_binomial_2_log travaille en log -> ar_pred est une log-moyenne -> l'AR(1) doit être en log
 
  vector<lower=0>[D_v] phi_disp_d = phi_disp_cluster[cluster_v] .* exp(tau_phi_disp * phi_disp_raw);
...
// prior vectorisé
  ...
  tau_phi_disp     ~ exponential(2); // forcer le shrinkage (si exponential(5) ) vers le niveau cluster M49, l'effet dyadique sortira s'il existe vraiment
